# Настройка контроллера на примере проекта **T1_PLCNext_Demo_**.
...
2. [Обновление прошивки контроллера через комндную оболочку](#Обновление-прошивки-контроллера-через-комндную-оболочку)
...
### Обновление прошивки контроллера через комндную оболочку
...
- Откройте комндную оболочку с помощью инструмента командной строки (например, **PuTTY**).
...
- В адресной строке ввести IP-адрес контроллера по умолчанию *10.0.16.73*
...
- Необходимо нажать **Proceed to 10.0.16.73 (unsafe)**
...
Запускаем PLCnext Engineer, открываем файл проекта, который находится в данном каталоге - **p:\PTUSA projects\projects\T1_PLCNext_Demo\PLCnext\Ptusa.pcwef**.
